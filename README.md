...
Все три модели были обучены на датасете mnist. Полные результаты обучения можно найти в файле "results/mnist_comparison/mnist_results.csv". Ниже я приведу выводы, полученные после анализа этого файла.
 - У всех моделей точность на train превышает значение в 0.99 (ResCNN - 0.9941, CNN - 0.9909, FC - 0.9927). На test лучшая точность у ResCNN (0.9922), на втором месте CNN (0.9920) и на третьем месте FC (0.9776).
...
Ниже представлена кривая обучения модели CNN. Я выбрал именно её, так как, как мне кажется, она имеет лучшие кривые обучения среди других двух моделей.
